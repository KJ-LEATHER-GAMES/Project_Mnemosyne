...
本書はProject MnemosyneにおけるTask本文、Output、Completion Criteria、task_statusの正本である。

`current-status.md` に記載されるTask情報は、現在地把握のための状態サマリーであり、Task定義の正本ではない。
...
|---|---|---|---|---|---|---|---|---|---|
| MNEMO-TASK-001 | P0 | Mnemosyne初期記憶5文書を作成する | Project Mnemosyne自身を、設計した記憶構造の最初の利用対象にする | `docs/phases/phase-1-memory-foundation.md` / M1-3 Active Templates | `project-summary.md` / `current-status.md` / `active-decisions.md` / `next-actions.md` / `ai-entrypoint.md` | 5文書のドラフトが揃い、新しいチャットでMnemosyneの現在地を再説明せずに相談開始できる | done | `docs/phases/phase-1-memory-foundation.md` | 2026-06-05 |
| MNEMO-TASK-002 | P0 | M1-4の5文書をレビューし、Active化に必要な修正点を洗い出す | ドラフトを正本候補として扱える品質へ引き上げる | M1-4 5文書ドラフト | レビュー指摘一覧 / 修正方針 | 不足・重複・責務逸脱・未決定事項混入が洗い出されている | done | `docs/projects/mnemosyne/memory/current-status.md` | 2026-06-05 |
| MNEMO-TASK-003 | P0 | M1-4 5文書のActive化用最終版を作成する | Mnemosyne自身の初期記憶を正式な参照対象にする | レビュー済みM1-4ドラフト / 修正指摘 | Active化用5文書 | `status: active` へ変更可能な内容になっている | done | `docs/projects/mnemosyne/memory/current-status.md` | 2026-06-05 |
...
M1-4 Mnemosyne初期記憶作成
  ├─ MNEMO-TASK-001: 5文書ドラフト作成 [done]
  ├─ MNEMO-TASK-002: 5文書レビュー [done]
  └─ MNEMO-TASK-003: Active化用最終版作成 [done]
       ↓
M1-5 ATS適用検証
...
| 1.0.0 | 2026-06-05 | active | M1-4 Active化レビューを反映し、MNEMO-TASK-001〜003をdoneへ更新。Task正本であることを明記してActive化。 | user |
