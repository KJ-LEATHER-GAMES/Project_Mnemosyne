...
現在は、Phase 1: Memory Foundation の M1-4「Mnemosyne初期記憶作成」を進行中である。

M1-4では、Project Mnemosyne自身を記憶構造の最初の利用対象として、以下の5文書を作成する。
...
AIがProject Mnemosyneについて相談・レビュー・文書作成を支援する場合、原則として以下の順序で参照する。

| Order | Document                                             | Purpose                                | When to Read   |
| ----- | ---------------------------------------------------- | -------------------------------------- | -------------- |
| 1     | `docs/projects/mnemosyne/memory/ai-entrypoint.md`    | 参照入口、読み順、制約、誤解しやすい点を把握する               | 常に最初           |
| 2     | `docs/projects/mnemosyne/memory/project-summary.md`  | プロジェクト目的、背景、範囲、正本方針を把握する               | 目的・全体像の確認時     |
| 3     | `docs/projects/mnemosyne/memory/current-status.md`   | 現在地、進行中タスク、Issue、Pending Decisionを把握する | 現在状況の確認時       |
| 4     | `docs/projects/mnemosyne/memory/active-decisions.md` | 現在有効なDecision / Constraintを確認する        | 判断・提案・レビュー前    |
| 5     | `docs/projects/mnemosyne/memory/next-actions.md`     | 次に実施すべきタスクと優先順位を確認する                   | 作業計画・次アクション提示時 |
...
| ----------------- | ---------------------------------------------------------------- | ------------------------------------------ |
| 「今どこまで進んでいる？」     | current-status.mdを基準に、現在Phase / milestone / in_progress taskを答える | `current-status.md`                        |
